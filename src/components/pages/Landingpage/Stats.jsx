...
const Stats = ({ data }) => {
    ...
    if (!statsData) return null; // 🚀 no stats at all → nothing rendered

    const headers = ["Type", "Oppkjøp", "Størrelse", "Område", "Tidsramme", "Resultat"];
        ...
        <div className="mx-auto px-4 sm:px-6 md:px-10">
          ...
          <div className="block sm:hidden space-y-4">
              <div className="grid grid-cols-2 gap-4 text-center">
                  {headers.map((header, index) =>
                      values[index] ? ( // ✅ render only if value exists
                          <div
                              key={`mobile-${index}`}
                              className={`p-4 ${index === 1 || index === 3 || index === 5 ? "" : "border-r border-[#aea293]/80"
                                  }`}
                          >
                              <TextAnimation
          ...
          <div className="hidden lg:block">
              <div className="grid grid-cols-6 text-center">
                  {headers.map((header, index) =>
                      values[index] ? ( // ✅ only show column if value exists
                          <div
                              key={`header-${index}`}
